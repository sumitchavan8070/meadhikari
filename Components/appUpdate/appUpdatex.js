...
const appUpdateFunction = async ({ appUpdate, context }) => {
  console.log(
    appUpdate?.softUpdate +
      appUpdate +
      "=-----------------------------------  here is app update func "
  );
  ...
  if (appUpdate?.softUpdate === 1 || appUpdate?.forceUpdate === 1) {
    if (updateAvailable) {
      ...
      updateDrawer({ appUpdate, context });
    }
...
const updateDrawer = ({ appUpdate, context }) => {
  if (
    Platform.OS === "android" &&
...
const appUpdateDialog = ({ context, appUpdate, launchStore }) => {
  Alert.alert(
